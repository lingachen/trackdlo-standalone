                        ...
                        double visibility_threshold) 
{
    ...
    for (int it = 0; it < max_iter; it ++) {
        ...
        if (pt2pt_dis(Y, Y_0 + G*W) / Y.rows() < tol) {
            Y = Y_0 + G*W;
            spdlog::info("Iteration until convergence: " + std::to_string(it+1));
            break;
        }
        else {
            Y = Y_0 + G*W;
        ...
        if (it == max_iter - 1) {
            spdlog::error("optimization did not converge!");
            converged = false;
            break;
        }
    }
...
void trackdlo::tracking_step (MatrixXd X_orig, 
    ...
    cpd_lle(X_orig, guide_nodes_, sigma2_pre_proc, beta_pre_proc_, lambda_pre_proc_, lle_weight_, mu_, max_iter_, tol_, false);

    if (visible_nodes_extended.size() == Y_.rows()) {
        if (visible_nodes.size() == visible_nodes_extended.size()) {
            spdlog::info("All nodes visible");
        }
        else {
            spdlog::info("Minor occlusion");
        }
        ...
        spdlog::info("Mid-section occluded");
        ...
        spdlog::info("Both ends occluded");
